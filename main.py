...
if __name__ == '__main__':
    ...
    train_dataset = MultimodalDataset(mydata, image_transform)
    ...
    for epoch in range(1):
        ...
        with torch.no_grad():
            for batch_text, batch_image, batch_label in train_dataloader:
                ...
                label_tensors = label_tensors.view(-1)

                text_predicted = torch.argmax(text_outputs,dim=1)
                text_correct += (text_predicted == label_tensors).sum().item()

                image_predicted = torch.argmax(image_outputs, dim=1)
                image_correct += (image_predicted == label_tensors).sum().item()
            ...
            print("text: ",text_correct/total)
            print("image: ",image_correct/total)